...
function renderMine(){
  ...
  E.cards.innerHTML=a.length?a.map(function(it){
    ...
    // simpla vazuta sau chiar deschisa... doi indicatori... cate
    // trimiteri a fost... fara scris, trebuie sa fie vizual clar"):
    // trei perechi icoana+numar, fara nicio eticheta scrisa - ochi
...
function renderFavorites(){
  ...
  function cardHtml(it){
    ...
    return '<article class="card" data-view-id="'+esc(it.id)+'">'+badge+cardImage(it)+'<div class="cardbody"><div class="cardtop"><div style="min-width:0;flex:1"><div class="title">'+esc(it.title)+'</div><div class="company">'+esc(it.company)+'</div></div><button class="fav on" data-fav="'+esc(it.id)+'">'+icon('heart')+'</button></div><div class="meta">'+icon('pin')+' '+esc(it.location)+'</div><div class="chips">'+(it.category?'<span class="chip">'+esc(it.category)+'</span>':'')+(it.price_label?'<span class="chip money">'+esc(it.price_label)+'</span>':'')+'</div><div class="cardactions"><span class="count">'+relativeTime(it.created_at)+'</span><div style="display:flex;gap:8px;align-items:center;"><button class="ghost sharebtn" data-share="'+esc(it.id)+'" title="Condividi">'+icon('share')+'</button><button class="details" data-detail="'+esc(it.id)+'">Dettagli →</button></div></div></div></article>';
  }
  ...
  E.cards.querySelectorAll('[data-share]').forEach(function(b){b.onclick=function(){shareAnnuncio(b.dataset.share)}});
  trackViewsOnScroll();
}
...
function relativeTime(x){var d=(Date.now()-new Date(x).getTime())/1000;if(d<60)return 'adesso';if(d<3600)return Math.floor(d/60)+' min fa';if(d<86400)return Math.floor(d/3600)+' ore fa';if(d<604800)return Math.floor(d/86400)+' giorni fa';if(d<2592000)return Math.floor(d/604800)+' settimane fa';if(d<31536000)return Math.floor(d/2592000)+' mesi fa';return Math.floor(d/31536000)+' anni fa'}
...
// fost vizualizat"): o vizualizare inseamna "a aparut pe ecran", nu
// neaparat deschis - IntersectionObserver e exact instrumentul facut
// pentru asta (standard, folosit de orice retea sociala pentru
// acelasi motiv). O singura data per anunt per vizita paginii (nu se
// re-numara daca dai scroll inainte-inapoi pe acelasi anunt) - un Set
// separat, care tine minte ce s-a numarat deja in aceasta sesiune.
var viewedIds=new Set();
function trackViewsOnScroll(){
  if(!('IntersectionObserver' in window))return;
  var obs=new IntersectionObserver(function(entries){
    entries.forEach(function(entry){
      if(entry.isIntersecting){
        var id=entry.target.dataset.viewId;
        if(id&&!viewedIds.has(id)&&!id.startsWith('demo')){
          viewedIds.add(id);
          apiCall('track_view',{id:id}).catch(function(){});
        }
        obs.unobserve(entry.target);
      }
    });
  },{threshold:0.5});
  E.cards.querySelectorAll('.card[data-view-id]').forEach(function(card){obs.observe(card)});
}

// Cerut direct ("fiecare anunt sa aiba sub el acel butonas care face
// sa trimita anuntul... cand se trimite apare si imaginea, ca pe
// WhatsApp"): distribuire prin functia nativa a telefonului
// (navigator.share), cu imaginea atasata ca FISIER real, nu doar un
// link - WhatsApp (si aplicatiile similare) arata direct fisierul
// atasat, garantat, spre deosebire de o previzualizare de link, care
// depinde de etichete speciale pe o pagina separata (imposibil de
// generat aici, fiind un site static, fara server care sa raspunda
...
// anterioara (10 KB) obliga compresia la o calitate foarte slaba
...
(function setupSwipe(){
  ...
  target.addEventListener('touchmove',function(e){
    ...
    if(Math.abs(dx)>60&&Math.abs(dx)>Math.abs(dy)*2){
      ...
      setTimeout(function(){
        state.tab=tabs[nextIdx][0];renderTabs();renderSide();fillCategories();load();
